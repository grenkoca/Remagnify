...
def parse_args():
    parser = argparse.ArgumentParser(description="Batch objective power standardization for WSIs.")

    parser.add_argument("-s", "--slides", type=str, dest="original_folder", required=True,
                        help="Folder where openslide supported files are stored. Images can be nested in subfolders.")
    parser.add_argument("-op", "--objective_powers", nargs='+', type=int, dest="objective_powers", required=True,
    ...
    parser.add_argument("-p", "--proxies", type=str, dest="proxy_folder", required=False,
...
if __name__ == "__main__":
    ...
    original_slides = sorted(glob(input_args.original_folder + "*.svs", recursive=True))

    # If a folder of proxy images is given, then pair up the original WSIs and proxy images.
    if input_args.proxy_folder is not None:
        cleaned_slides = sorted(glob(input_args.proxy_folder + "*.tiff", recursive=True))
        # If there is a different number of proxies and cleaned slides, raise an assertion.
        assert len(original_slides) == len(cleaned_slides), "%d original slide files found, but %d proxies found."

        # Pair proxies and slides up and make sure they have the same slide ID
        for slide_path, cleaned_path in list(zip(original_slides, cleaned_slides)):
            slide_id_original = slide_path.split(".svs")[0].split("/")[-1]
            slide_id_cleaned = cleaned_path.split("_overlay.tiff")[0].split("/")[-1]
            ...
            for objective_power, folder in resolution_folder_dict.items():
                ...
                slide_object.save_downsampled(output_file, objective_power, blur_image=False, use_proxy=True)
    else:
        for slide_path in original_slides:
            ...
            for objective_power, folder in resolution_folder_dict.items():
                os.makedirs(folder, exist_ok=True)
                output_file = folder + slide_id_original + ".tiff"
                slide_object.save_downsampled(output_file, objective_power, blur_image=False, use_proxy=False)
