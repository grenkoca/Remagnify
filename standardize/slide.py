...
class Slide(OpenSlide):
    ...
    def add_proxy(self, fpath):
        assert os.path.isfile(fpath), "File not found (or is directory): %s" % fpath
    ...
    def gather_image_magnifications(self):
        ...
        for level in range(self.level_count):
            if len(level_dim_factors) == 0:
                ...
                base_level_dim_proportion = self.level_dimensions[0][0] / self.level_dimensions[0][1]
                estimated_magnification = float(obj_pow)
                self.estimated_magnifications.append(estimated_magnification)
            else:
                inverse_downsample_factor = self.level_dimensions[level][0] / self.level_dimensions[0][0]
                level_dim_proportion = self.level_dimensions[level][0] / self.level_dimensions[level][1]
                ...
                self.estimated_magnifications.append(round(estimated_magnification, 2))
            print("", end="")
            print(str(level), end="\t\t")
            print(str(self.level_dimensions[level]), end="\t\t")
            print(str(round(estimated_magnification, 2)), end="\n")

    def save_downsampled(self, fpath, new_obj_power, blur_image=False, use_proxy=False):
        # TODO: Use memmap?
        if use_proxy:
            ...
            original_obj_power = self.estimated_magnifications[0]  # assume base image is highest resolution
        else:
            if float(new_obj_power) in self.estimated_magnifications:
                print("\tUsing magnification found in slide object")
                obj_power_index = self.estimated_magnifications.index(new_obj_power)
                slide_image = self.read_region((0, 0), obj_power_index, self.level_dimensions[obj_power_index])
        ...
        if blur_image:
            ...
            fp = np.memmap(filename, dtype=np.uint8, mode="w+", shape=slide_image.shape)
            fp[:] = slide_image[:]
            slide_image = gaussian(fp, sigma=GAUSSIAN_SIGMA, preserve_range=True, multichannel=True)

        fold_change = new_obj_power / original_obj_power
        new_dimensions = int(slide_image.shape[0] * fold_change), int(slide_image.shape[1] * fold_change)
        step = round(1/fold_change)
        print("Step size:", step)
        downsampled_image = slide_image[::step, ::step, :]

        print("\tNew dimensions for %sx -> %sx: %s (actual: %s)" %
              (original_obj_power, new_obj_power, new_dimensions, downsampled_image.shape))
        try:
